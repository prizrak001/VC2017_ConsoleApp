...
BOOST_VERSION_REGEX="^([0-9]{1,5})\.([0-9]{2,5})\.([0-9]{1,5})$"

[[ "$BOOST_VERSION" =~ $BOOST_VERSION_REGEX ]] || {
    echo "${CURRENT_SCRIPT}. Error 2. Incorrect Boost version: '${BOOST_VERSION}'" >&2
    exit 1
}

echo "${CURRENT_SCRIPT}. Boost version: ${BOOST_VERSION}" >&2

BOOST_VERSION_1="${BASH_REMATCH[1]}"
BOOST_VERSION_2="${BASH_REMATCH[2]}"
BOOST_VERSION_3="${BASH_REMATCH[3]}"

BOOST_ARCHIVE_FILENAME="boost_${BOOST_VERSION_1}_${BOOST_VERSION_2}_${BOOST_VERSION_3}.tar.bz2"
